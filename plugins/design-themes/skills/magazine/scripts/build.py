...
# Content-filled demos are the primary verification source for the core trio.
...
def build_pptx(name: str = "slides") -> bool:
    ...
    if result.returncode != 0:
        ...
        return False
    # The script writes output.pptx in cwd; move it to the target demo path.
...
CN_PRIMARY_FONTS = {"TsangerJinKai02", "Newsreader"}
EN_PRIMARY_FONTS = {"TsangerJinKai02", "Newsreader"}
...
def verify_target(name: str, source: str, max_pages: int, src_dir: Path) -> list[str]:
    ...
    if not any(exp in font_name for exp in expected for font_name in embedded):
        ...
        fallback_present = any(
            ...
            for kw in ("TsangerJinKai02", "Newsreader", "SourceSerif", "SourceHan", "Noto", "Georgia", "Charter")
        )
...
DISALLOWED_KINGHWA = re.compile(r"KingHwa|京華|OldSong", re.IGNORECASE)
MIXED_LIGHT_PAGE = re.compile(r'class="[^"]*(?:page|slide)[^"]*\blight\b[^"]*"')
LIGHT_SLIDE_CALL = re.compile(r'new_slide\(\s*prs,\s*"light"', re.DOTALL)
...
def scan_file(path: Path) -> list[Finding]:
    ...
    for i, raw in enumerate(lines, start=1):
        ...
        if DISALLOWED_KINGHWA.search(raw):
            findings.append(Finding(path, i, "kinghwa-font",
                                    "KingHwa / 京華老宋体 is not allowed in guizang-magazine outputs"))

        if MIXED_LIGHT_PAGE.search(raw):
            findings.append(Finding(path, i, "mixed-light-page",
                                    "Default guizang outputs must stay single-mode dark; use light only when explicitly requested"))

        if path.suffix == ".py" and LIGHT_SLIDE_CALL.search(raw):
            findings.append(Finding(path, i, "default-light-slide",
                                    "Default PPT templates must use dark slides unless light mode is explicitly requested"))
