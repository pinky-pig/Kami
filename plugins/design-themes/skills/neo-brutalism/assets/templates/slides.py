#!/usr/bin/env python3
"""
Kami Neo-Brutalism slide deck generator.
...
NAVY_SOFT = RGBColor(0x00, 0x00, 0x00)
PAPER = RGBColor(0xFF, 0xFD, 0xF5)
PAPER_LIGHT = RGBColor(0xFF, 0xFF, 0xFF)
IVORY = RGBColor(0xFF, 0xFF, 0xFF)
INK = RGBColor(0x00, 0x00, 0x00)
COPY = RGBColor(0x00, 0x00, 0x00)
MUTED = RGBColor(0x1F, 0x1F, 0x1F)
STONE = RGBColor(0x00, 0x00, 0x00)
RULE = RGBColor(0x00, 0x00, 0x00)
TAG = RGBColor(0xFF, 0xD9, 0x3D)
...
def slide_cover(prs):
    s = framed_slide(prs, "REPUBLICAN MANUSCRIPT EDITION", 1)
    title_plaque(
        s,
        INNER_X + Inches(0.72),
        INNER_Y + Inches(0.78),
    ...
    metric_card(s, INNER_X + Inches(9.9), INNER_Y + Inches(2.82), Inches(1.55), "01", "hard border", "black ink")
    add_text(
        s,
        "2026.04 · Kami Fork",
        INNER_X + Inches(0.74),
...
def slide_visual_language(prs):
    ...
    section_header(
        ...
        2,
        "Neo-Brutalism 视觉语言",
        "不做复古海报，不做重纹理，只把专业文档整理成被认真归档过的样子。",
        3,
    )
    swatches = [
        ("Ink Black", NAVY, "#000000", "边框 / 文字 / 阴影"),
        ("Hot Red", RGBColor(0xFF, 0x6B, 0x6B), "#FF6B6B", "题签 / 强调块"),
        ("Vivid Yellow", TAG, "#FFD93D", "标签 / 提示 / 卡片底"),
        ("Border", RULE, "#D0C7BB", "细线和分隔"),
    ]
    ...
    archive_card(
        ...
        Inches(0.88),
        "装饰边界",
        "所有结构必须可见：粗黑边、零模糊硬阴影和高饱和色块是主视觉；不要回到柔和 SaaS 卡片。",
    )
...
def slide_templates(prs):
    ...
    cards = [
        ...
        ("Resume", "固定坐标版简历样例，适合验证蓝框档案式布局。", "demo"),
