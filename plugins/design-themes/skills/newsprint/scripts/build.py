...
# Verification uses templates directly. Keep demo HTML out of the default path
# so stale sample content cannot override the active Newsprint structure.
...
CN_PRIMARY_FONTS = {
    "TsangerJinKai02", "Newsreader", "SourceSerif", "SourceHan", "Noto",
    "Charter", "Georgia", "TimesNewRoman",
}
EN_PRIMARY_FONTS = {"TsangerJinKai02", "Newsreader", "SourceSerif", "Charter", "Georgia", "TimesNewRoman"}
...
def verify_target(name: str, source: str, max_pages: int, src_dir: Path) -> list[str]:
    ...
    if not any(exp in font_name for exp in expected for font_name in embedded):
        ...
        fallback_present = any(
            ...
            for kw in ("TsangerJinKai02", "Newsreader", "SourceSerif", "SourceHan", "Noto", "Georgia", "Charter", "TimesNewRoman")
        )
