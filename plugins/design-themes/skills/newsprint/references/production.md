...
```css
font-family: "Newsreader", "Source Han Serif SC", "Noto Serif CJK SC",
...
中文字体栈：
- Serif：`Newsreader` -> `Source Han Serif SC` -> `宋体`
...
输出里如果看到 `DejaVuSerif` / `Bitstream Vera`，说明指定字体没生效，走到了系统兜底。正确应该看到 `Newsreader` 或 `Source Han Serif SC`。
...
```bash
# 把 .ttf 放 HTML 同目录
cp Newsreaderv2.002.ttf workspace/
