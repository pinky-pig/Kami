#!/usr/bin/env python3
"""kami Playful Geometric build & check

Usage:
    python3 scripts/build.py                      # build all 15 examples (10 HTML + 3 diagrams + 2 PPTX + Slidev)
...
# Theme templates may resolve to bundled fonts or macOS CJK/system fallbacks.
CN_PRIMARY_FONTS = {
    "KingHwa_OldSong", "Songti", "Hiragino", "PingFang",
    "SourceHan", "Noto", "Inter", "Arial",
}
...
def verify_target(name: str, source: str, max_pages: int, src_dir: Path) -> list[str]:
    ...
    if not any(exp in font_name for exp in expected for font_name in embedded):
        ...
        fallback_present = any(
            ...
            for kw in ("Newsreader", "Inter", "KingHwa_OldSong", "SourceHan", "Noto", "Georgia", "Charter", "Songti", "Hiragino", "PingFang", "Arial")
        )
...
def scan_file(path: Path) -> list[Finding]:
    ...
    if path.name == "one-pager.html":
        if "sticker-face" not in text:
            findings.append(Finding(path, 1, "missing-offset-dom",
                                    "one-pager must use real .sticker-face DOM for PDF-stable offset borders"))
