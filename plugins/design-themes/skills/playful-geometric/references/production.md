...
### Playful Geometric offset border rule

不要用 `box-shadow`、多重 shadow、或纯 `::before/::after` 伪元素来画核心偏移边框。WeasyPrint 对这些组合的层级、分页和多层硬边并不稳定。
...
外层负责右下偏移色块和黑边，`.sticker-face` 负责正面卡片、主黑边、圆角和内容 padding。这样 HTML 和 PDF 渲染一致，也不会因为 offset 被计算成额外分页。
