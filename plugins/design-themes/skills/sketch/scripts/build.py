...
# Theme templates may resolve to bundled fonts or macOS handwritten fallbacks.
CN_PRIMARY_FONTS = {"LXGW", "Tsanger", "Marker", "Bradley", "Noteworthy", "PingFang"}
EN_PRIMARY_FONTS = {"Marker", "Bradley", "Noteworthy", "ComicSans"}
...
def verify_target(name: str, source: str, max_pages: int, src_dir: Path) -> list[str]:
    ...
    if not any(exp in font_name for exp in expected for font_name in embedded):
        ...
        fallback_present = any(
            ...
            for kw in ("LXGW", "Tsanger", "Marker", "Bradley", "Noteworthy", "ComicSans", "PingFang", "Arial")
        )
...
# Theme-specific palettes are allowed; keep only structural checks here.
COOL_GRAY_BLOCKLIST = set()
...
LINE_HEIGHT_LOOSE = re.compile(r"line-height\s*:\s*2\.\d*", re.IGNORECASE)
...
def scan_file(path: Path) -> list[Finding]:
    ...
    if path.name == "one-pager.html":
        if "radial-gradient(var(--muted) 1px, transparent 1px)" not in text:
            findings.append(Finding(path, 1, "missing-paper-texture",
                                    "one-pager must include dotted paper texture"))
        if "box-shadow: 4px 4px 0 0 var(--border)" not in text and "box-shadow: 4px 4px 0px 0px var(--border)" not in text:
            findings.append(Finding(path, 1, "missing-hard-shadow",
                                    "one-pager must include hard offset sketch shadows"))
        if "255px 15px 225px 15px / 15px 225px 15px 255px" not in text:
            findings.append(Finding(path, 1, "missing-wobbly-radius",
                                    "one-pager must include wobbly border radius values"))
        if "grid-template-columns: repeat(3, 1fr)" not in text:
            findings.append(Finding(path, 1, "one-pager-too-dense",
                                    "one-pager must default to 3 metrics, not 4"))
    return findings
