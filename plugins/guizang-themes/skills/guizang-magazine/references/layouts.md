...
- 当前生产默认使用全黑色 `dark` 模式，PPT / HTML / PDF 不再黑白页切换
- 如果用户明确要求亮色，必须作为独立的全亮输出模式整体切换，不能在同一产物内混排
...
- 非 hero 页使用 `dark`；hero 页使用 `hero dark`
...
**核心机制**:默认每页都使用 `dark` 或 `hero dark`。不要在同一份 deck / PDF / HTML 里穿插 `light` 页；这会让输出变成“一页黑色、一页亮色”的机械节奏。

#### 按布局的主题默认值

| Layout | 默认主题 | 原因 |
|---|---|---|
| 1. 开场封面 | `hero dark` | 开场仪式感来自标题尺度和留白 |
| 2. 章节幕封 | `hero dark` | 通过 chrome、标题和密度变化制造转场 |
| 3. 大字报(数据) | `dark` | 数字与规则线在黑底上形成电子墨水反差 |
| 4. 左文右图 | `dark` | 正文保持稳定阅读底色 |
| 5. 图片网格 | `dark` | 图片区域使用深色卡片或反相占位 |
| 6. Pipeline | `dark` | 流程图需清晰 |
| 7. 问题页 | `hero dark` | 通过大标题与留白制造冲击 |
| 8. 大引用 | `dark` | 金句靠尺度、规则线和位置建立仪式感 |
| 9. 对比页 | `dark` | 双列需清晰 |
| 10. 图文混排 | `dark` | 保持图片与正文一致的纸面底色 |

#### 节奏硬规则(生成后 grep 自检)

- ❌ **禁止**在默认输出里穿插 `light` 页
- ❌ **禁止**用黑白切换当作节奏
- ✅ **推荐**每 3-4 页插入 1 个 hero(封面/幕封/问题/大引用)，节奏来自页面密度、标题尺度、规则线和组件组合
...
**先画这张表对齐,再动手写 slide**。跳过规划直接粘骨架 = 密度失控。
...
- 两个幕的封面可以切换 `hero dark` / `hero dark`，制造节奏
...
1. **Hero Cover**（第 1 页）
2. **Act Divider**（第一幕开场，hero dark 或 hero dark）
