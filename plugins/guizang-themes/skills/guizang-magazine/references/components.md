...
每一页都是一个 `<section class="slide ...">`。当前生产版默认使用单一黑色主题，避免 PPT / HTML / PDF 在黑色页和亮色页之间机械交替。

```html
<section class="slide dark" data-theme="dark">   <!-- 默认深色页 -->
<section class="slide dark hero" data-theme="dark">  <!-- Hero 页：深色 + 薄遮罩透出 WebGL -->
```

**主题使用**：默认全程 `dark`。如用户明确指定亮色，必须作为独立的全亮主题模式实现，而不是在同一份产物里交替穿插。
