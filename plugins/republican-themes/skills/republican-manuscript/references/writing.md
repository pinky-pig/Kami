...
**结构模板**：
1. 蓝色函件题签（Filed Letter / Recommendation Letter）
...
3. **每个项目 1-2 页**：
   - 项目标题 + 类型 tag + 时间
   - 一句话描述 "做了什么"
   - 视觉位默认先保留纯色占位；只有在用户明确提供 / 要求图片时再换成 `<img>`
   - 角色 + 挑战 + 结果
...
- 视觉第一，文字辅助
- 每个项目的结果必须可量化
- 如果表达的是关系 / 流程 / 优先级，优先嵌入 `assets/diagrams/` 的架构图 / 流程图 / 象限图
- 真实图片是可选增强，不是模板初始化前提
- demo 重生成也遵守同一契约：默认保留纯色占位和 diagrams 位，不自动往首页或项目页注入大图
