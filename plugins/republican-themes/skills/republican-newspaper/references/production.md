...
<style>
@font-face {
  font-family: "<assets/fonts/fonts.json serif.latin>";
  src: url("<assets/fonts/fonts.json serif.file>");
}
body { font-family: "<assets/fonts/fonts.json serif.latin>", serif; }
</style>
```
...
中文字体栈从 `assets/fonts/fonts.json` 读取；不要把字体名重复写进生成脚本。PPTX 必须同时写 Latin / East Asian / Complex Script 三个槽位，其中中文走配置里的 `eastAsian`。
...
def font_slot(role, slot="latin"):
    return json.loads(FONT_CONFIG.read_text(encoding="utf-8"))[role][slot]
    ...
    for tag in ("latin", "ea", "cs"):
        for child in list(r_pr):
            if child.tag.endswith("}" + tag):
                r_pr.remove(child)
    for tag, face in (("latin", font), ("ea", SERIF_EA), ("cs", font)):
...
输出里如果看到 `DejaVuSerif` / `Bitstream Vera`，说明指定字体没生效，走到了系统兜底。正确应该看到 `assets/fonts/fonts.json` 中配置的 serif 字体，或一个明确的中文 serif fallback。
...
```bash
# 把 fonts.json 中声明的 .ttf 放 HTML 同目录
cp path/to/configured-serif.ttf workspace/
